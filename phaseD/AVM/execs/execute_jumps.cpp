...
equality_check equal_check_dispatcher[] = { 
    check_eq_number ,
    check_eq_strings
};

jump_cmp_func jump_dispatcher[] = { 
    jge_impl,
    jgt_impl,
    jle_impl,
    jlt_impl
...
void execute_rljmp(instruction * instr){
    ...
    if (rv1->type != number_m || rv2->type != number_m) {
        ...
        if (dispatcher_idx < (sizeof(jump_dispatcher) / sizeof(jump_cmp_func))) {
            double comparison_value = jump_dispatcher[dispatcher_idx](rv1->data.numVal, rv2->data.numVal);
            comparison_result_bool = (comparison_value != 0.0);
        } else {
            avm_error("Internal error: Relational jump opcode out of dispatcher bounds.");
            executionFinished = 1;
        }
    }
