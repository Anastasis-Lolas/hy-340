...
std::vector<std::string> local_string_vec_consts;
unsigned int total_strings;
infile.read(reinterpret_cast<char*>(&total_strings), sizeof(unsigned int));
std::cout << "\n--- String Constants (Read from file: " << total_strings << ") ---" << std::endl;
...
local_string_vec_consts.push_back(s);
std::cout << i << ": \"" << s << "\"\n";
}

std::vector<int> local_int_vec_consts;
unsigned int total_numbers_int;
infile.read(reinterpret_cast<char*>(&total_numbers_int), sizeof(unsigned int));
std::cout << "\n--- Integer Constants (Read from file: " << total_numbers_int << ") ---" << std::endl;
for (unsigned int i = 0; i < total_numbers_int; ++i) {
int val_int;
infile.read(reinterpret_cast<char*>(&val_int), sizeof(int));
local_int_vec_consts.push_back(val_int);
std::cout << i << ": " << val_int << "\n";
}

std::vector<double> local_double_vec_consts;
unsigned int total_numbers_double;
infile.read(reinterpret_cast<char*>(&total_numbers_double), sizeof(unsigned int));
std::cout << "\n--- Double Constants (Read from file: " << total_numbers_double << ") ---" << std::endl;
for (unsigned int i = 0; i < total_numbers_double; ++i) {
double d;
infile.read(reinterpret_cast<char*>(&d), sizeof(double));
local_double_vec_consts.push_back(d);
std::cout << i << ": " << d << "\n";
}

std::vector<userfunc> local_user_funcs;
unsigned int total_user_funcs;
infile.read(reinterpret_cast<char*>(&total_user_funcs), sizeof(unsigned int));
std::cout << "\n--- User Functions (Read from file: " << total_user_funcs << ") ---" << std::endl;
...
local_user_funcs.push_back(uf);
std::cout << i << ": ID=" << uf.id << ", Address=" << uf.address << ", Locals=" << uf.localSize << "\n";
}

std::vector<std::string> local_lib_funcs;
unsigned int total_lib_funcs;
infile.read(reinterpret_cast<char*>(&total_lib_funcs), sizeof(unsigned int));
std::cout << "\n--- Library Functions (Read from file: " << total_lib_funcs << ") ---" << std::endl;
...
local_lib_funcs.push_back(s_lib);
std::cout << i << ": " << s_lib << "\n";
}

std::cout << "\n--- Target Code (AVM Instructions Read from file) ---" << std::endl;
...
std::cout << std::left << std::setw(col_num_width_r) << "No."
<< std::left << std::setw(col_opcode_width_r) << "Opcode"
<< std::left << std::setw(col_arg_width_r) << "Result"
<< std::left << std::setw(col_arg_width_r) << "Arg1"
<< std::left << std::setw(col_arg_width_r) << "Arg2"
<< std::endl;
int total_width_for_separator_r = col_num_width_r + col_opcode_width_r + (col_arg_width_r * 3);
...
std::cout << std::left << std::setw(col_opcode_width_r) << vmopcode_to_string(current_opcode);
...
if (arg_to_print->type >= label_a && arg_to_print->type <= undef_a) {
...
case number_a:
if (arg_to_print->val < local_double_vec_consts.size()) {
extra_info = " (" + std::to_string(local_double_vec_consts[arg_to_print->val]) + ")";
} else if (arg_to_print->val < local_int_vec_consts.size()){
extra_info = " (" + std::to_string(local_int_vec_consts[arg_to_print->val]) + ")";
}
break;
case string_a:
if (arg_to_print->val < local_string_vec_consts.size())
extra_info = " (\"" + local_string_vec_consts[arg_to_print->val] + "\")";
break;
case userfunc_a:
if (arg_to_print->val < local_user_funcs.size())
extra_info = " (" + local_user_funcs[arg_to_print->val].id + ")";
break;
case libfunc_a:
if (arg_to_print->val < local_lib_funcs.size())
extra_info = " (" + local_lib_funcs[arg_to_print->val] + ")";
break;
case bool_a:
extra_info = (arg_to_print->val != 0 ? " (true)" : " (false)");
break;
default: break;
}
...
std::cout << std::left << std::setw(col_arg_width_r) << vmarg_to_display_string(&result_read);
std::cout << std::left << std::setw(col_arg_width_r) << vmarg_to_display_string(&arg1_read);
std::cout << std::left << std::setw(col_arg_width_r) << vmarg_to_display_string(&arg2_read);
std::cout << "\n";
}
std::cout << std::string(total_width_for_separator_r, '-') << std::endl;
std::cout << "===========================================================\n";
